...
public interface StuffRepository extends GraphRepository<Stuff> {
    ...
    @Query("UNWIND {categories} AS cat MATCH (c:Category) WHERE id(c)=cat MATCH(s:Stuff) WHERE id(s)={id} CREATE (s)-[:has]->(c)")
    void addRelationshipsWithCategories(@Param("categories") Set<Long> categories, @Param("id") Long id);

    @Query("UNWIND {users} AS user MATCH (u:User) WHERE id(u)=user  MATCH(s:Stuff) WHERE id(s)={id} CREATE (u)-[:use]->(s)")
    void addRelationshipsWithUsers(@Param("users") Set<Long> users, @Param("id") Long id);

    @Query("MATCH (u:User) WHERE id(u)={user} MATCH(s:Stuff) WHERE id(s)={id} CREATE (u)-[:create]->(s) CREATE (u)-[:use]->(s)")
